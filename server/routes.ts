...
Return ONLY the enhanced bio text, no explanations or quotes.`;
  ...
  // Text-to-speech using OpenAI TTS API (high quality voices)
  app.post("/api/tts", isAuthenticated, async (req: any, res: Response) => {
    try {
      ...
      // Use OpenAI TTS with nova voice - warm, conversational tone
      const response = await openai.audio.speech.create({
        model: "tts-1-hd", // High definition model for better quality
        voice: "nova", // Nova has a warm, friendly female voice
        input: truncatedText,
        response_format: "mp3",
      });
